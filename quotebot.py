...
def is_owner():
    return c.check(lambda ctx: ctx.message.author.id == "146925011503022080") # add owner ID here
...
async def new_quote(name, keyword, body: str):
    ...
    if name in quotes:  # check if user is in the database
        ...
        return await my_bot.say("That person is not in the database. Please add them first.")
